...
# 요청(request)과 응답(response) 상이에 특정 작업 수행
# 미들웨어는 모든 요청에 대해 실행되며, 요청을 처리하기 전에 응답을 반환하기 전에 특정 작을을 수행할 수 있음.
# 예를 들어 로기으 인증, cors처리, 압축등...
import logging # 로그 출력용

from starlette.requests import Request
from starlette.responses import Response

app = FastAPI( # 생성자를 통해서 postman을 대체하는 문서화를 내장되어 있음
...
class LoggingMiddleware(BaseHTTPMiddleware): # 로그를 콘솔에 출력하는 용도
    ...
    async def dispatch(self, request, call_next): # 로그 출력 추가
...
class Item(BaseModel):              # 아이템 객체 생성(BaseModel : 객체 연결)
    name : str                      # 상품명 : 문자열
